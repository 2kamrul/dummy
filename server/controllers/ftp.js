...
var csvFile = {
    formatA: ['/I_EVTSCAN.csv'],
    formatB: ['/I_SHIFTEVT.CSV'],
    formatC: ['/I_STATUS.csv']
}

// var csvFiles = ['/I_EVTSCAN.csv', '/I_SHIFTEVT.CSV', '/I_STATUS.csv']
// var csvFiles = ['/I_EVTSCAN.csv', '/I_STATUS.csv']
// var csvFiles = ['/I_STATUS.csv', 'I_DOFFPRD.CSV']
var csvFiles = [
    {
        name: 'I_STATUS',
        path: '/I_STATUS.csv'
...
// var csvFiles = ['I_DOFFPRD.CSV']

const serverIP = [
    { ip: '192.168.114.23', name: 'machine1' },
    { ip: '192.168.114.23', name: 'machine2' }
]
    ...
    try {
        for (const server of serverIP) {
            for (const file of csvFiles) {
                await ftpSingleFileRead(file.path, file.name, server.ip, server.name)
            }
        }
        // for (const file of csvFiles) {
        //     await fptUpdate(file.path, file.name)
...
const fptUpdate = async (path, name) => {
...
const ftpSingleFileRead = async (path, name, server_ip, server_name) => {
